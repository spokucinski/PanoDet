...
def removeVerticalLine(image: cv2.typing.MatLike, x: int, sourceImage: cv2.typing.MatLike, lineThickness:int = 5):
    image[:, x-lineThickness:x+lineThickness] = sourceImage[:, x-lineThickness:x+lineThickness]

def addStatusInfo(image: cv2.typing.MatLike, x: int, imageIndex: int, maxImageIndex: int, lastSuggestedX: int = None):
    if lastSuggestedX:
        cv2.putText(img=image, text=f"X = {x}, Image: {imageIndex} / {maxImageIndex}, Sug.: {lastSuggestedX}", org=(100, 100), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=(0, 0, 255), thickness=3, lineType=cv2.LINE_AA)    
    else:
...
def removeStatusInfo(image: cv2.typing.MatLike, sourceImage: cv2.typing.MatLike):
    image[0:int(0.1*image.shape[0]), 0:int(0.5*image.shape[1])] = sourceImage[0:int(0.1*image.shape[0]), 0:int(0.5*image.shape[1])]
